// copied js from Codepen pinning Locomotive scrollTrigger
function loco(){
...
// creating canvas function and putting all the img links inside `var data`
...
function scaleImage(img, ctx) {
  ...
  var vRatio = canvas.height / img.height;          //protrait img = Math.min, Landscape img= Math.max
  var ratio = Math.min(hRatio, vRatio);             // if img available is better area at landscape, use max..but if img is portrait based, use Math.min to set area(to load hair of girl properly)
  var centerShift_x = (canvas.width - img.width * ratio) / 2;   //make the canvas center positioned
  var centerShift_y = (canvas.height - img.height * ratio) / 2;
  ctx.clearRect(0, 0, canvas.width, canvas.height);     //when going onwards, then clear previous images
  ctx.drawImage(
    img,
    0,
    0,
    ...
    img.height,
    centerShift_x,
    centerShift_y,                      // this function loads next images
    img.width * ratio,
    img.height * ratio
  );
}
...
function canvas1(){
  ...
  var tl3 = gsap.timeline({     //inside Glass sensor Off n On
    scrollTrigger:{
        trigger:`#page21`,
        start:`top top`,
        scrub:1,
  ...
  var tl4 = gsap.timeline({     //outside Glass sensor Off n On
    scrollTrigger:{
        trigger:`#page22`,
        start:`top top`,
        scrub:1,
